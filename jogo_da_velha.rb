...
def change_turn(num)
    ...
    if @count.odd? && @count < 10 
        ...
        p "GAME OVER"
        p "Quer jogar mais uma vez? Clique S para sim ou N para não:"
        after_match = gets.chomp
        if after_match == 'S' || after_match == 's'
            make_new_match()
        elsif after_match == 'N' || after_match == 'n'
            abort('Obrigado por jogar!')
        end 
    end
end
...
def end_game(string)
    if string == ' X '
        p "#{@player1.upcase} WINS!"
        @count = 10

    elsif string == ' O '
        p "#{@player2.upcase} WINS!"
        @count = 10
